...
export const addToFavorites = createAsyncThunk(
  ...
  (movie, thunkAPI) => {
    ...
    return axios
      .put(
        `/api/movies/addFavorite?userId=${users.id}&code=${movie.id}&title=${movie.title}&poster_path=${movie.poster_path}&vote_average=${movie.vote_average}&release_date=${movie.release_date}`,
        {
          code: movie.id,
          title: movie.title,
...
export const UserFavorites = createAsyncThunk(
  "USER_FAVORITES",
  ({ id, setMovies }) => {
    return axios
...
export const FavoritesRow = createAsyncThunk(
  "FAVORITES_ROW",
  (setFavorites, thunkAPI) => {
    const { users } = thunkAPI.getState();
    if (!users.id) throw new Error("You need to be logged in");
    return axios
